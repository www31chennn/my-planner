...
// ── Year Goals ─────────────────────────────────────────────
function YearGoals({ user, saving, setSaving, year }) {
  ...
  useEffect(() => {
    ...
    setLoaded(false);
    cachedReadOne(user, "goals", String(year)).then(val => {
  ...
  function save(next) {
    ...
    timer.current = setTimeout(async () => {
      await writeOne(user, "goals", String(year), JSON.stringify(next));
...
// ── Month Planner ──────────────────────────────────────────
function MonthPlanner({ user, saving, setSaving, year }) {
  ...
  async function loadMonth(m) {
    ...
    const val = await cachedReadOne(user, "month", key);
  ...
  function saveMonth(m, data) {
    ...
    timers.current[key] = setTimeout(async () => {
      await writeOne(user, "month", key, JSON.stringify(data));
...
// ── Daily Note ─────────────────────────────────────────────
function DailyNote({ user, saving, setSaving }) {
  ...
  useEffect(() => {
    ...
    cachedReadOne(user, "daily", date).then(val => {
  ...
  function save(text) {
    ...
    clearTimeout(timer.current);
    timer.current = setTimeout(async () => {
      if (text) await writeOne(user, "daily", date, text);
      else await deleteOne(user, "daily", date);
...
function PlannerApp({ user, saving, setSaving, plannerName, onOpenSettings }) {
      ...
      <div style={{ flex:1, overflow:"hidden" }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        <div style={{ display:"flex", width:"300%", height:"100%", transform:`translateX(${-tab*33.333}%)`, transition:"transform 0.38s cubic-bezier(0.4,0,0.2,1)" }}>
          <div style={{ width:"33.333%", height:"100%", overflowY:"auto", flexShrink:0 }}>
            <YearGoals user={user} saving={saving.goals} setSaving={setSaving} year={year} />
          </div>
          <div style={{ width:"33.333%", height:"100%", overflowY:"auto", flexShrink:0 }}>
            <MonthPlanner user={user} saving={saving.month} setSaving={setSaving} year={year} />
          </div>
          <div style={{ width:"33.333%", height:"100%", overflowY:"auto", flexShrink:0 }}>
            <DailyNote user={user} saving={saving.daily} setSaving={setSaving} />
          </div>
        </div>
      </div>
